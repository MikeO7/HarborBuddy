...
test-cover: ## Run tests with aggregate and per-package coverage gates
	...
	@coverage="$$(go tool cover -func=coverage.out | awk '/^total:/ {gsub(/%/, "", $$3); print $$3}')"; \
		awk -v coverage="$$coverage" 'BEGIN { if (coverage + 0 < 100) { printf "coverage %.1f%% is below 100%%\n", coverage; exit 1 } }'
...
lint-docker: ## Lint the Dockerfile with pinned Hadolint
	$(CONTAINER_ENGINE) run --rm --volume "$(CURDIR):/work:ro" \
		--entrypoint hadolint \
		docker.io/hadolint/hadolint:v2.15.1-alpine@sha256:a1d49ae1a4e83c1dbad26b8c1ad7588c8bd1e04f4866b34ad3cac50335198552 \
