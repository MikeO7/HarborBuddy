...
func TestRunUpdateCycleRejoinsAnActiveSelfUpdateHandoff(t *testing.T) {
	self := docker.ContainerSummary{ID: "self", Name: "harborbuddy", ImageRef: "harborbuddy:latest", ImageID: "sha256:old"}
	helper := docker.ContainerSummary{ID: "existing-helper", Name: "harborbuddy-updater", Labels: map[string]string{
		docker.SelfUpdateHelperLabel: "true",
		docker.SelfUpdateTargetLabel: self.ID,
	}}
	client := &selfUpdateFakeClient{
		containers: []docker.ContainerSummary{self, helper},
		details:    map[string]docker.ContainerDetails{"self": runningDetails(self)},
		images:     map[string]docker.ImageInfo{"harborbuddy:latest": {ID: "sha256:new"}},
	}
	originalDetector := detectCurrentContainer
	detectCurrentContainer = func([]docker.ContainerSummary) string { return self.ID }
	defer func() { detectCurrentContainer = originalDetector }()

	_, err := RunUpdateCycle(context.Background(), config.Default(), client, zerolog.Nop())
	signal, ok := selfupdate.AsShutdownRequired(err)
	if !ok || signal.HelperContainerID != helper.ID || signal.TargetContainerID != self.ID {
		t.Fatalf("RunUpdateCycle() error = %v, want existing helper handoff", err)
	}
	if len(client.actions) != 0 {
		t.Fatalf("rejoined handoff started duplicate work: %v", client.actions)
	}
}
