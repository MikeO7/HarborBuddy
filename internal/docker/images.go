...
func (d *DockerClient) PullImage(ctx context.Context, imageRef string) (ImageInfo, error) {
	reader, err := d.cli.ImagePull(ctx, imageRef, imagetypes.PullOptions{})
	if err != nil {
...
func (d *DockerClient) ListImages(ctx context.Context) ([]ImageInfo, error) {
	images, err := d.cli.ImageList(ctx, imagetypes.ListOptions{All: true})
	if err != nil {
		return nil, fmt.Errorf("list images: %w", err)
	}
	return imageSummaries(images, false), nil
}

func (d *DockerClient) ListDanglingImages(ctx context.Context) ([]ImageInfo, error) {
	args := filters.NewArgs(filters.Arg("dangling", "true"))
	images, err := d.cli.ImageList(ctx, imagetypes.ListOptions{All: true, Filters: args})
	if err != nil {
		return nil, fmt.Errorf("list dangling images: %w", err)
	}
	return imageSummaries(images, true), nil
}
...
func (d *DockerClient) RemoveImage(ctx context.Context, imageID string) error {
	if _, err := d.cli.ImageRemove(ctx, imageID, imagetypes.RemoveOptions{Force: false, PruneChildren: true}); err != nil {
