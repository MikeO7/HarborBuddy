...
func TestSelfUpdateHelperConfigIsRestricted(t *testing.T) {
	current := ContainerDetails{
		...
		Networks: map[string]*network.EndpointSettings{
			"app": {NetworkID: "network-id", EndpointID: "endpoint-id", IPAddress: "172.20.0.5", Aliases: []string{"harborbuddy"}},
		},
	}
	...
	if config.Image != request.TargetImageID || !slices.Equal([]string(config.Entrypoint), []string{"/harborbuddy"}) {
	...
	if endpoint.NetworkID != "" || endpoint.EndpointID != "" || endpoint.IPAddress != "" || endpoint.Aliases != nil {
